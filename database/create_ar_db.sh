...
#
#    ./database/create_ar_db.sh .env
#

echo "=== Database setup for AR backend ==="
...
echo "Note that the AR_DATABASE_USER and AR_DATABASE_PASSWORD read from the .env file are the ones that will be created by this script,"
...
if [ -z "$AR_DATABASE_NAME" ] || [ -z "$AR_DATABASE_USER" ] || [ -z "$AR_DATABASE_PASSWORD" ]; then
    echo "ERROR: Missing required database configuration in '.env' file."
    echo "Please ensure AR_DATABASE_NAME, AR_DATABASE_USER, and AR_DATABASE_PASSWORD are set."
    echo "Note that the AR_DATABASE_USER and AR_DATABASE_PASSWORD are the ones that will be created by this script,"
    echo "not the superuser credentials that will be used by this script to connect to the postgres server."
    exit 1
fi
...
CREATE DATABASE ${AR_DATABASE_NAME};
-- Revoke all default privileges from PUBLIC in this database
REVOKE ALL ON DATABASE ${AR_DATABASE_NAME} FROM PUBLIC;

CREATE USER ${AR_DATABASE_USER} WITH
...
GRANT CONNECT ON DATABASE ${AR_DATABASE_NAME} TO ${AR_DATABASE_USER};
\c ${AR_DATABASE_NAME}
-- Lock down the public schema completely
REVOKE ALL ON SCHEMA public FROM PUBLIC;
GRANT USAGE, CREATE ON SCHEMA public TO ${AR_DATABASE_USER};

-- Your app privileges (these are perfect)
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO ${AR_DATABASE_USER};
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO ${AR_DATABASE_USER};
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO ${AR_DATABASE_USER};
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON SEQUENCES TO ${AR_DATABASE_USER};
SQL_EOF
)

echo "Setting up PostgreSQL database '${AR_DATABASE_NAME}' and new user '${AR_DATABASE_USER}' as specified in the .env file..."

if [ "$AR_DATABASE_HOST" = "localhost" ] || [ "$AR_DATABASE_HOST" = "127.0.0.1" ]; then
    echo "Using peer auth as system user postgres for local database server at host '$AR_DATABASE_HOST'..."
