...
test('switch rating dimensions across songs', async ({ page }) => {
  ...
  await page.locator('.arw-dimension-buttons button', { hasText: 'arousal' }).click();
  await expect(legendItems).toHaveCount(5);

  await page.locator('#song-list .song-nav', { hasText: '2. Demo Song 2' }).click();

  await page.locator('.arw-dimension-buttons button', { hasText: 'enjoyment' }).click();
  await expect(legendItems).toHaveCount(10);

  await page.locator('.arw-dimension-buttons button', { hasText: 'is_cool' }).click();
