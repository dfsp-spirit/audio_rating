...
def check_frontend_audio_files(
    frontend_dir: str, studies_config_json_file: str
) -> bool:
    """
    ...
    if not config_path.exists():
        print(
            f"ERROR: Studies configuration file '{studies_config_json_file}' does not exist"
        )
        return False
    if not config_path.is_file():
        print(
            f"ERROR: Studies configuration path '{studies_config_json_file}' is not a file"
        )
        return False
    ...
    for study in cfg.studies:
        ...
        for song in study.songs_to_rate:
            ...
            if not audio_file_path.exists():
                ...
                print(
                    f"WARNING: Path '{audio_file_path}' exists but is not a file (might be a directory)"
                )
                ...
                found_files.append({"study": study_name, "file": media_file})
    ...
    for study_name, stats in study_stats.items():
        ...
        print(
            f"{study_name[:30]:<30} {stats['found']:<8} {stats['total']:<8} {status:<10} ({percentage:.1f}%)"
        )
    ...
    for study_name, stats in study_stats.items():
        if stats["total"] == 0:
            print(
                f"⚠️ WARNING: Study '{study_name}' has no audio files specified. Please check if this is intentional or a misconfiguration."
            )
