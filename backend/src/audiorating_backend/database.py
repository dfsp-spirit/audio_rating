...
def create_config_file_studies(config_path: str):
    ...
    config = load_studies_config(
        config_path
    )  # will raise FileNotFoundError if not found, which is fine. That file is required.
                ...
                for participant_id in study_cfg.study_participant_ids:
                    ...
                    if not existing_link:
                        ...
                        participant_link = StudyParticipantLink(
                            study_id=new_study.id, participant_id=participant.id
                        )
                        session.add(participant_link)
                        logger.info(
                            f"Added pre-listed participant to study: {participant_id}"
                        )
                ...
                for dim_index, dimension_cfg in enumerate(study_cfg.rating_dimensions):
                    ...
                    existing_dimension = session.exec(
                        select(StudyRatingDimension).where(
                            ...
                            StudyRatingDimension.dimension_title
                            == dimension_cfg.dimension_title,
                        )
                    ).first()
                    ...
                    if not existing_dimension:
                        ...
                        logger.info(
                            f"Added rating dimension: {dimension_cfg.dimension_title}"
                        )

                # Commit all relationships for this study
                session.commit()
                logger.info(
                    f"Successfully created study '{study_cfg.name_short}' with {len(study_cfg.songs_to_rate)} songs and {len(study_cfg.rating_dimensions)} rating dimensions"
                )
            else:
...
def report_on_db_contents():
    ...
    with Session(engine) as session:
        ...
        for study in studies:
            ...
            logger.info(
                f"Study '{study.name_short}' has {len(song_links)} songs and {len(rating_dims)} rating dimensions."
            )
            ...
            for song_link in song_links:
                ...
                if song:
                    logger.info(
                        f" - Song: {song.display_name} ({song.media_url}): {song.description}"
                    )

            # Log rating dimensions
            for dimension in rating_dims:
                logger.info(
                    f" - Rating Dimension: {dimension.dimension_title} ({dimension.num_values} values, min={dimension.minimal_value}, default={dimension.default_value}): {dimension.description}"
                )

            # report whether study allows unlisted participants
            logger.info(
                f" - Allows unlisted participants: {study.allow_unlisted_participants}"
            )

            if not study.allow_unlisted_participants:
                incomplete_participants = get_participant_ids_missing_ratings_for_study(
                    study.name_short
                )

                logger.info(
                    " - Incomplete participants (have at least one rating with missing values):"
                )
                # print the invitation link for each incomplete participant
                for participant_id in incomplete_participants:
                    try:
                        invitation_link = get_invitation_link_for_study_and_participant(
                            study.name_short, participant_id
                        )
                        logger.info(
                            f"   - Participant ID: {participant_id}, Invitation Link: {invitation_link}"
                        )
                    except ValueError as e:
                        logger.warning(
                            f"   - Participant ID: {participant_id}, Error generating invitation link: {e}"
                        )
            else:
                logger.info(
                    " - Study allows unlisted participants, so no specific participant IDs to report."
                )
                logger.info(
                    f" - Invitation link for participant 'example_participant_id': {get_invitation_link_for_study_and_participant(study.name_short, 'example_participant_id')}"
                )
    ...
    with Session(engine) as session:
        ...
        for participant_id in participant_ids:
            ...
            complete_ratings_count = session.exec(
                ...
                .where(
                    Rating.participant_id == participant_id, Rating.study_id == study.id
                )
                .where(
...
def get_invitation_link_for_study_and_participant(
    study_name_short: str, participant_id: str
) -> str:
    ...
    with Session(engine) as session:
        ...
        if not study.allow_unlisted_participants:
            ...
            if not participant_link:
                raise ValueError(
                    f"Participant '{participant_id}' is not allowed in study '{study_name_short}'"
                )
